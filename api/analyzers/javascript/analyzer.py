...
class JavaScriptAnalyzer(AbstractAnalyzer):
    ...
    def process_class_definition(
        ...
        if (
            ...
            and body_node.child(0).type == "expression_statement"
        ):
        ...
        # Create Class object
        c = Class(
            str(path), class_name, docstring, node.start_point[0], node.end_point[0]
        )

        return (c, inherited_classes)

    def process_function_definition(
        self, node: Node, path: Path, source_code: str
    ) -> Function:
        """
        ...
        if (
            body
            and body.child_count > 0
            and body.children[0].type == "expression_statement"
        ):
        ...
        f = Function(
            str(path), function_name, docstring, ret_type, src, start_line, end_line
        )
    ...
    def first_pass_traverse(
        self,
        parent: Union[File, Class, Function],
        node: Node,
        path: Path,
        graph: Graph,
        source_code: str,
        ...
        Args:
            ...
            graph (Graph): The graph to add the processed entities to.
            source_code (str): The source code of the file being analyzed.
        """
        entity = None
        ...
        if node.type == "class_declaration":
            ...
            if entity.name == "constructor":
                entity.ret_type = parent.name
            graph.add_function(entity)

        elif node.type == "method_definition":
            entity = self.process_method_definition(node, path, source_code)
            graph.add_function(entity)
    ...
    def first_pass(self, path: Path, f: io.TextIOWrapper, graph: Graph) -> None:
        ...
        logger.info(f"JavaScript Processing {path}")
    ...
    def process_function_call(self, node) -> Optional[str]:
        ...
        """
        # First try to get the callee directly
        callee = node.child_by_field_name("function")
        if callee:
            if callee.type == "identifier":
                return callee.text.decode("utf-8")
            elif callee.type == "member_expression":
                # Handle method calls like console.log
        ...
        elif sibling.type == "member_expression":
            # Handle method calls
            property_node = sibling.child_by_field_name("property")
            if property_node:
                callee_name = property_node.text.decode("utf-8")
        else:
            logger.warning(f"Unknown function call pattern: {sibling.type}")
            return None
    ...
    def process_call_node(
        self, caller: Union[Function, File], callee_name: str, graph: Graph
    ) -> None:
        """
    ...
    def process_inheritance(
        self, cls: Class, super_classes: list[str], graph: Graph
    ) -> None:
    ...
    def second_pass(self, path: Path, f: io.TextIOWrapper, graph: Graph) -> None:
        """
        Perform a second pass analysis on the given JavaScript file.
